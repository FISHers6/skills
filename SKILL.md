...
| Static slide renders as blank white page | `transpile.py` only parsed `effect.baseLayer`; when `effects` is empty it created an empty root layer instead of reading `event.baseLayer` | For `effect = none`, call `parse_layer(event['baseLayer'])` so non-animated slides still emit their full layer tree. |
| Horizontal spoon/chopsticks render vertical or drift out of place | Static orientation lived in `initialState.rotation` / `initialState.scale` (and sometimes constant `transform.rotation.z` with identical from/to), but `generate_tsx.py` only emitted changing transforms | Parse `rotation` / `scale` from `initialState` in `parse_layers.py` and always emit non-zero static rotation in `generate_tsx.py`, even when the rotation is not animated. |
| Transition slide loses dark background or static props (only animated ring/text remains) | `effect.baseLayer` is often only the animated subtree, not the full slide scene. Rendering it directly drops static siblings from `event.baseLayer` such as the dark background, chopsticks, spoon, etc. | In `transpile.py`, always start from `event.baseLayer` for transition slides too. Merge the animated subtree from `effect.baseLayer` back into the matching node in the full scene, using `objectID` as the primary key and a structural signature fallback. Then parse the merged tree. |
